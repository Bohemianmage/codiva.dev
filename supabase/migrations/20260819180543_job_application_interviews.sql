...
CREATE TABLE public.ops_job_interview_rounds (
  ...
  CONSTRAINT ops_job_interview_rounds_kind_ck CHECK (
    kind IN ('screening', 'technical', 'culture', 'final', 'other')
  ),
  CONSTRAINT ops_job_interview_rounds_status_ck CHECK (
    status IN ('planned', 'done', 'skipped')
  ),
  CONSTRAINT ops_job_interview_rounds_outcome_ck CHECK (
    outcome IS NULL OR outcome IN ('advance', 'hold', 'reject')
  ),
  CONSTRAINT ops_job_interview_rounds_title_len_ck CHECK (
...
COMMENT ON TABLE public.ops_job_interview_rounds IS
  'Rondas de entrevista internas por postulación. El estado interview de la postulación sigue siendo el semáforo del pipeline.';
...
CREATE TABLE public.ops_job_interview_comments (
  ...
  body text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT ops_job_interview_comments_body_len_ck CHECK (
...
COMMENT ON TABLE public.ops_job_interview_comments IS
  'Notas internas de entrevista; append-only, firmadas por el staff que escribe.';
...
CREATE POLICY careers_review_select_ops_job_interview_rounds ON public.ops_job_interview_rounds
  FOR SELECT
  USING (public.is_careers_review_staff());

CREATE POLICY careers_review_insert_ops_job_interview_rounds ON public.ops_job_interview_rounds
  FOR INSERT
  WITH CHECK (public.is_careers_review_staff());

CREATE POLICY careers_review_update_ops_job_interview_rounds ON public.ops_job_interview_rounds
  FOR UPDATE
  USING (public.is_careers_review_staff())
  WITH CHECK (public.is_careers_review_staff());

CREATE POLICY careers_review_delete_ops_job_interview_rounds ON public.ops_job_interview_rounds
  FOR DELETE
  USING (public.is_careers_review_staff());

CREATE POLICY admin_select_ops_job_interview_comments ON public.ops_job_interview_comments
  FOR SELECT
  USING (public.is_admin_staff());

CREATE POLICY careers_review_select_ops_job_interview_comments ON public.ops_job_interview_comments
  FOR SELECT
  USING (public.is_careers_review_staff());

CREATE POLICY admin_insert_ops_job_interview_comments ON public.ops_job_interview_comments
  FOR INSERT
  WITH CHECK (
...
CREATE POLICY careers_review_insert_ops_job_interview_comments ON public.ops_job_interview_comments
  FOR INSERT
  WITH CHECK (
...
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.ops_job_interview_rounds TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.ops_job_interview_rounds TO service_role;
GRANT SELECT, INSERT ON TABLE public.ops_job_interview_comments TO authenticated;
GRANT SELECT, INSERT ON TABLE public.ops_job_interview_comments TO service_role;
