...
export default async function InterviewsAcceptPage() {
  ...
  // Staff preview (incl. “ver como”) no pasa por el muro legal del tercero.
  if (access.isStaffPreview) redirect(homeHref);
  if (!access.member) redirect(homeHref);
  if (getAcceptanceStatus(access.member).complete) redirect(homeHref);
  const t = await getT();
